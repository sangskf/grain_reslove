...
pub fn send_hex_data(
    ...
    debug!("命令调用: send_hex_data - IP: {}, 端口: {}, 超时: {:?}", ip, port, timeout_ms);
    info!("准备发送数据到 {}:{}, 数据长度: {} 字节", ip, port, data.len());
    ...
    match &result {
        Ok(response) => {
            ...
            info!("成功接收来自 {}:{} 的响应，长度: {} 字节，耗时: {:?}", 
                  ip, port, response_len, elapsed);
            debug!("响应数据内容: {}", response_preview);
        },
        Err(e) => {
            error!("发送数据到 {}:{} 失败: {}, 耗时: {:?}", ip, port, e, elapsed);
        }
    }
...
pub fn get_logs(level: Option<String>, limit: Option<usize>) -> Result<Vec<LogEntry>, String> {
    debug!("命令调用: get_logs - 级别过滤: {:?}, 限制数量: {:?}", level, limit);
    
    let actual_limit = limit.unwrap_or(100);
    info!("正在获取日志，级别过滤: {:?}，限制数量: {}", level, actual_limit);
...
pub fn add_log(level: &str, message: &str) -> Result<(), String> {
    debug!("命令调用: add_log - 级别: {}, 消息长度: {}", level, message.len());
